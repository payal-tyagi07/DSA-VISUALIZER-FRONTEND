...
const CountSetBits = () => {
  ...
  const [num, setNum] = useState(5); // 0101
  ...
  const generateSteps = (n, width) => {
    ...
    for (let i = width - 1; i >= 0; i--) {
      const bit = (n >> i) & 1;
      if (bit === 1) count++;
      steps.push({ i, bit, count });
    }
    steps.push({ final: true, total: count });
    return steps;
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Count Set Bits</h2>
      ...
      <div className="bg-[#0d0d0d] p-3 rounded-lg border-l-4 border-[#569cd6] mb-4">
        <p className="text-gray-300">
          {step.final
            ? `Total set bits = ${step.total}`
            : `Bit ${step.i}: ${step.bit} → current count = ${step.count}`}
        </p>
      </div>

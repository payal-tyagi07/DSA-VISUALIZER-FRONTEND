...
const UniquePathsObstacles = () => {
  const pseudocodeLines = [
    "procedure uniquePathsWithObstacles(obstacleGrid):",
    "  m = rows, n = cols",
    "  dp = 2D array of size m x n, filled with 0",
  ...
  const [input, setInput] = useState('0,0,0;0,1,0;0,0,0');
  ...
  const generateSteps = (grid) => {
    ...
    steps.push({ dp: dp.map(row => [...row]), m, n, grid, description: "Initialize dp[0][0]", pseudocodeLine: 2 });
    for (let i = 0; i < m; i++) {
      for (let j = 0; j < n; j++) {
        ...
        if (i > 0) dp[i][j] += dp[i-1][j];
        if (j > 0) dp[i][j] += dp[i][j-1];
        steps.push({ dp: dp.map(row => [...row]), m, n, grid, current: [i,j], description: `dp[${i}][${j}] = ${dp[i][j]} (from ${i>0?`top=${dp[i-1][j]}`:''} ${j>0?`left=${dp[i][j-1]}`:''})`, pseudocodeLine: 4 });
      }
    }
    steps.push({ dp: dp.map(row => [...row]), m, n, grid, result: dp[m-1][n-1], description: `Total paths = ${dp[m-1][n-1]}` });
    return steps;
  };
  ...
  const rowLabels = Array.from({length: step.m}, (_,i)=>`i=${i}`);
  const colLabels = Array.from({length: step.n}, (_,i)=>`j=${i}`);

  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Unique Paths with Obstacles</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(m*n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(m*n)</span></div></div>
...
export default UniquePathsObstacles;
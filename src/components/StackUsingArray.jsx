...
const StackUsingArray = () => {
  const pseudocodeLines = [
    ...
    "  arr = []",
    "  push(x): arr.append(x)",
    "  pop(): return arr.pop() if arr else None",
    "  top(): return arr[-1] if arr else None",
  ...
  const generateSteps = (ops) => {
    const steps = [];
    let stack = [];
    steps.push({ stack: [...stack], description: "Start with empty stack", pseudocodeLine: 0 });
    for (let i = 0; i < ops.length; i++) {
      const op = ops[i];
      if (op.startsWith('push')) {
        const val = parseInt(op.split(' ')[1]);
        stack.push(val);
        steps.push({ stack: [...stack], description: `Push ${val} onto stack`, pseudocodeLine: 2 });
      } else if (op === 'pop') {
        const popped = stack.length > 0 ? stack.pop() : null;
        steps.push({ stack: [...stack], description: `Pop → ${popped !== null ? popped : 'stack empty'}`, pseudocodeLine: 3 });
      } else if (op === 'top') {
        const topVal = stack.length > 0 ? stack[stack.length-1] : null;
        steps.push({ stack: [...stack], top: topVal, description: `Top = ${topVal !== null ? topVal : 'stack empty'}`, pseudocodeLine: 4 });
      } else if (op === 'isEmpty') {
        const empty = stack.length === 0;
        steps.push({ stack: [...stack], empty, description: `isEmpty = ${empty}`, pseudocodeLine: 5 });
      }
    }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Stack using Array</h2>

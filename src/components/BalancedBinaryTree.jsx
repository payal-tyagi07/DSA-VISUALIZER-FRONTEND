...
const BalancedBinaryTree = () => {
  ...
  const [input, setInput] = useState('3,9,20,null,null,15,7');
  ...
  const generateSteps = (arr) => {
    ...
    const height = (node) => {
      if (!node) return 0;
      steps.push({ root, highlight: new Set([node]), description: `Check height of node ${node.val}` });
      const left = height(node.left);
      steps.push({ root, highlight: new Set([node]), description: `Left height of ${node.val} = ${left}` });
      if (left === -1) { result = false; return -1; }
      const right = height(node.right);
      steps.push({ root, highlight: new Set([node]), description: `Right height of ${node.val} = ${right}` });
      if (right === -1) { result = false; return -1; }
      if (Math.abs(left - right) > 1) {
        steps.push({ root, highlight: new Set([node]), description: `Height difference ${Math.abs(left-right)} > 1 → not balanced` });
        return -1;
      }
      const h = 1 + Math.max(left, right);
      steps.push({ root, highlight: new Set([node]), description: `Node ${node.val} height = ${h}` });
      return h;
    };
    const h = height(root);
    if (h === -1) result = false;
    steps.push({ root, result, description: `Tree is ${result ? 'balanced' : 'not balanced'}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Balanced Binary Tree</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(n)</span></div></div>
      <div className="flex flex-wrap justify-center gap-4 mb-6"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-80" /><button onClick={updateTree} className="bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-1 rounded">Update</button></div>
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          {step.root && <TreeRenderer root={step.root} highlightNodes={step.highlight} />}
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]"><p className="text-gray-300"><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {step.description}</p>{step.result !== undefined && <p className="text-[#4ec9b0] mt-2">Result = {step.result ? 'Balanced' : 'Not balanced'}</p>}</div>

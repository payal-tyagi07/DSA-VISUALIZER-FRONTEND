...
const GroupAnagrams = () => {
  const pseudocodeLines = [
    "procedure groupAnagrams(strs):",
    "  map = new Map()",
    "  for s in strs:",
    "    key = sort(s) // or count array",
    "    map[key].push(s)",
  ...
  const generateSteps = (strs) => {
    const steps = [];
    const words = strs.split(',').map(s => s.trim());
    const groups = new Map();
    steps.push({ words, description: "Group anagrams", pseudocodeLine: 1 });
    for (let i = 0; i < words.length; i++) {
      const w = words[i];
      const key = w.split('').sort().join('');
      steps.push({ words, highlight: [i], key, description: `Word "${w}" → sorted key = "${key}"`, pseudocodeLine: 2 });
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key).push(w);
      steps.push({ words, groups: Array.from(groups.values()), description: `Added to group: ${groups.get(key)}`, pseudocodeLine: 3 });
    }
    steps.push({ words, result: Array.from(groups.values()), description: `Groups: ${JSON.stringify(Array.from(groups.values()))}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Group Anagrams</h2>
        <div className="flex justify-center gap-4 mb-4">
          <span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm border border-[#3c3c3c]">⏱️ Time: O(n * k log k)</span>
          <span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm border border-[#3c3c3c]">💾 Space: O(n*k)</span>
        </div>
      </div>

      <div className="flex justify-center gap-4 mb-6">
        <input type="text" value={input} onChange={(e) => setInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-96" />
      </div>
        ...
        <div className="lg:w-2/3 space-y-6">
          <div className="flex justify-center gap-3 flex-wrap">
            {step.words?.map((word, idx) => (
              <div key={idx} className={`p-2 rounded ${getHighlight(idx)}`}>{word}</div>
            ))}
          </div>
          {step.groups && (
            <div className="bg-[#1e1e1e] p-4 rounded">
              <p className="text-[#9cdcfe]">Current groups:</p>
              {step.groups.map((g, i) => <div key={i} className="text-gray-300">[{g.join(', ')}]</div>)}
            </div>
          )}

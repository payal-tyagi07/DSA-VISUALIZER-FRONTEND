...
const KthLargestElement = () => {
  ...
  const generateSteps = (arr, kVal) => {
    const steps = [];
    let heap = [];
    steps.push({ nums: arr, k: kVal, heap: [], description: "Start with empty min heap", pseudocodeLine: 0 });
    for (let i = 0; i < arr.length; i++) {
      const num = arr[i];
      heap.push(num);
      steps.push({ nums: arr, heap: [...heap], description: `Insert ${num} into heap`, pseudocodeLine: 2 });
      // Bubble up to maintain min heap (simplified, we just show the heap after insertion)
      heap.sort((a,b)=>a-b); // keep as min heap for visualization
      if (heap.length > kVal) {
        const removed = heap.shift();
        steps.push({ nums: arr, heap: [...heap], description: `Heap size > ${kVal}, remove smallest (${removed})`, pseudocodeLine: 3 });
      }
      steps.push({ nums: arr, heap: [...heap], description: `After processing ${num}, heap = [${heap.join(',')}]`, pseudocodeLine: 2 });
    }
    const result = heap[0];
    steps.push({ result, description: `The ${kVal}th largest element is ${result}`, pseudocodeLine: 4 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Kth Largest Element</h2>
        ...
        <div className="lg:w-2/3 space-y-6">
          <div className="flex gap-1 flex-wrap">
            {step.nums?.map((val, idx) => (
              <div key={idx} className="w-12 h-12 flex items-center justify-center text-lg font-mono rounded border bg-[#1e1e1e] text-white border-[#3c3c3c]">
                {val}
              </div>
            ))}
          </div>
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">Min Heap (size ≤ k)</div>

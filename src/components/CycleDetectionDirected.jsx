...
const CycleDetectionDirected = () => {
  ...
  const graph = {
    ...
    edges: [
      ...
      { from: 'E', to: 'B' } // creates a cycle B->C->D->E->B
    ]
  };
  ...
  const generateSteps = () => {
    ...
    const dfs = (node) => {
      visited.add(node);
      recStack.add(node);
      steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), recStack: new Set(recStack), currentNode: node, description: `Visit ${node} (add to recursion stack)`, pseudocodeLine: 6 });
      const neighbors = graph.edges.filter(e => e.from === node).map(e => e.to);
      for (const neighbor of neighbors) {
        if (!visited.has(neighbor)) {
          steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), recStack: new Set(recStack), currentNode: node, highlightNodes: new Set([neighbor]), description: `Check neighbor ${neighbor} (not visited)`, pseudocodeLine: 7 });
          if (dfs(neighbor)) return true;
        } else if (recStack.has(neighbor)) {
          steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), recStack: new Set(recStack), currentNode: node, highlightNodes: new Set([neighbor]), description: `Found back edge to ${neighbor} (in recursion stack) → cycle detected`, pseudocodeLine: 8 });
          return true;
        }
      }
      recStack.delete(node);
      steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), recStack: new Set(recStack), currentNode: node, description: `Backtrack from ${node} (remove from recursion stack)`, pseudocodeLine: 9 });
      return false;
    };
    ...
    steps.push({ nodes: graph.nodes, edges: graph.edges, visited, recStack, cycleFound, description: `Graph ${cycleFound ? 'contains' : 'does not contain'} a directed cycle`, pseudocodeLine: 4 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Cycle Detection (Directed Graph)</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(V+E)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(V)</span></div></div>
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]"><p className="text-gray-300"><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {step.description}</p><p className="text-sm text-gray-400">Recursion stack: [{step.recStack ? Array.from(step.recStack).join(', ') : ''}]</p>{step.cycleFound !== undefined && <p className="text-[#4ec9b0] mt-2">Result: {step.cycleFound ? 'Cycle detected' : 'No cycle'}</p>}</div>
...
export default CycleDetectionDirected;
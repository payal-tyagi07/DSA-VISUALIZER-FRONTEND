...
const CoinChangeII = () => {
  const pseudocodeLines = [
    "procedure change(amount, coins):",
    "  dp = array of size amount+1, dp[0]=1",
    "  for coin in coins:",
  ...
  const generateSteps = (coins, amount) => {
    ...
    dp[0] = 1;
    steps.push({ dp: [...dp], description: "Initialize dp[0]=1", pseudocodeLine: 1 });
    for (let coin of coins) {
      for (let i = coin; i <= amount; i++) {
        dp[i] += dp[i-coin];
        steps.push({ dp: [...dp], current: i, coin, description: `Using coin ${coin}: dp[${i}] += dp[${i-coin}] = ${dp[i]}`, pseudocodeLine: 2 });
      }
    }
    steps.push({ dp: [...dp], result: dp[amount], description: `Number of combinations = ${dp[amount]}` });
    return steps;
  };
    ...
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          {step.coin && <div className="text-sm text-gray-400">Processing coin: {step.coin}</div>}

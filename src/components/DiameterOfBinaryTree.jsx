...
const DiameterOfBinaryTree = () => {
  ...
  const generateSteps = (arr) => {
    ...
    const height = (node) => {
      if (!node) return 0;
      steps.push({ root, highlight: new Set([node]), description: `At node ${node.val}, compute left and right heights` });
      ...
      if (cand > diameter) {
        diameter = cand;
        steps.push({ root, highlight: new Set([node]), description: `Path through ${node.val} has length ${cand} → new diameter = ${diameter}` });
      }
      const h = 1 + Math.max(left, right);
      steps.push({ root, highlight: new Set([node]), description: `Height of ${node.val} = ${h}` });
      return h;
    };
    height(root);
    steps.push({ root, diameter, description: `Diameter of tree = ${diameter}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Diameter of Binary Tree</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(n)</span></div></div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]"><p className="text-gray-300"><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {step.description}</p>{step.diameter !== undefined && <p className="text-[#4ec9b0] mt-2">Diameter = {step.diameter}</p>}</div>

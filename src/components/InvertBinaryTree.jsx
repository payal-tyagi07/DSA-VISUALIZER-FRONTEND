...
const InvertBinaryTree = () => {
  ...
  const defaultTree = [4,2,7,1,3,6,9];
  const [treeArr, setTreeArr] = useState(defaultTree);
  const [steps, setSteps] = useState([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1000);
  const [input, setInput] = useState('4,2,7,1,3,6,9');
  ...
  const generateSteps = (arr) => {
    ...
    const invert = (node) => {
      if (!node) return null;
      steps.push({ root, highlight: new Set([node]), description: `Swap children of node ${node.val}` });
      const temp = node.left;
      node.left = node.right;
      node.right = temp;
      steps.push({ root, highlight: new Set([node]), description: `After swap: left = ${node.left?.val ?? 'null'}, right = ${node.right?.val ?? 'null'}` });
      invert(node.left);
      invert(node.right);
      return node;
    };
    const newRoot = invert(root);
    steps.push({ root: newRoot, description: `Tree inverted` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Invert Binary Tree</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(n)</span></div></div>

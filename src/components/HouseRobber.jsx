...
const HouseRobber = () => {
  ...
  const generateSteps = (arr) => {
    const steps = [];
    const n = arr.length;
    if (n === 0) return [{ dp: [], result: 0, description: "Empty array → 0" }];
    if (n === 1) return [{ dp: [arr[0]], result: arr[0], description: "Only one house → rob it" }];
    const dp = new Array(n);
    dp[0] = arr[0];
    dp[1] = Math.max(arr[0], arr[1]);
    steps.push({ dp: [...dp], description: "Base cases: dp[0] = first house, dp[1] = max(first, second)", pseudocodeLine: 2 });
    for (let i = 2; i < n; i++) {
      dp[i] = Math.max(dp[i-1], dp[i-2] + arr[i]);
      steps.push({ dp: [...dp], current: i, description: `dp[${i}] = max(dp[${i-1}]=${dp[i-1]}, dp[${i-2}]+nums[${i}]=${dp[i-2]}+${arr[i]}=${dp[i]})`, pseudocodeLine: 3 });
    }
    steps.push({ dp: [...dp], result: dp[n-1], description: `Maximum amount = ${dp[n-1]}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">House Robber</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(n)</span></div></div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          <div className="flex gap-2 flex-wrap justify-center mb-4">
            {nums.map((val, i) => <div key={i} className="w-16 h-16 flex items-center justify-center text-xl font-mono rounded border bg-[#1e1e1e] text-white border-[#3c3c3c]">{val}</div>)}
          </div>

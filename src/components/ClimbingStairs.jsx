...
const ClimbingStairs = () => {
  ...
  const generateSteps = (n) => {
    ...
    if (n >= 1) dp[1] = 1;
    if (n >= 2) dp[2] = 2;
    steps.push({ dp: [...dp], description: "Initialize dp[1]=1, dp[2]=2", pseudocodeLine: 2 });
    for (let i = 3; i <= n; i++) {
      dp[i] = dp[i-1] + dp[i-2];
      steps.push({ dp: [...dp], current: i, description: `dp[${i}] = dp[${i-1}] + dp[${i-2}] = ${dp[i-1]} + ${dp[i-2]} = ${dp[i]}`, pseudocodeLine: 3 });
    }
    steps.push({ dp: [...dp], result: dp[n], description: `Number of ways = ${dp[n]}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Climbing Stairs</h2>

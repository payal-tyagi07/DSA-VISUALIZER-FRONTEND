...
const ContainerWithMostWater = () => {
  ...
  const generateSteps = (arr) => {
    ...
    steps.push({ heights: arr, left, right, maxArea, description: "Start", pseudocodeLine: 1 });
    while (left < right) {
      const width = right - left;
      const height = Math.min(arr[left], arr[right]);
      const area = height * width;
      if (area > maxArea) maxArea = area;
      steps.push({ heights: arr, left, right, area, maxArea, description: `Width = ${width}, height = ${height} → area = ${area}, maxArea = ${maxArea}`, pseudocodeLine: 3 });
      if (arr[left] < arr[right]) {
        left++;
        steps.push({ heights: arr, left, right, description: `Move left pointer to ${left}`, pseudocodeLine: 4 });
      } else {
        right--;
        steps.push({ heights: arr, left, right, description: `Move right pointer to ${right}`, pseudocodeLine: 5 });
      }
    }
    steps.push({ result: maxArea, description: `Maximum water = ${maxArea}`, pseudocodeLine: 6 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Container With Most Water</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">Heights</div>
            <div className="flex items-end gap-1">
              {step.heights?.map((h, idx) => (
                <div key={idx} className="flex flex-col items-center">
                  <div className={`w-12 transition-all duration-300 ${getHighlight(idx)} bg-blue-500`} style={{ height: `${h * 6}px`, minHeight: '20px' }}></div>

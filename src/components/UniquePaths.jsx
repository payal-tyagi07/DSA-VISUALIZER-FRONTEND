...
const UniquePaths = () => {
  ...
  const generateSteps = (rows, cols) => {
    const steps = [];
    const dp = Array(rows).fill().map(() => Array(cols).fill(0));
    for (let i = 0; i < rows; i++) dp[i][0] = 1;
    for (let j = 0; j < cols; j++) dp[0][j] = 1;
    steps.push({ dp: dp.map(row => [...row]), rows, cols, description: "Initialize first row and column to 1", pseudocodeLine: 2 });
    for (let i = 1; i < rows; i++) {
      for (let j = 1; j < cols; j++) {
        dp[i][j] = dp[i-1][j] + dp[i][j-1];
        steps.push({ dp: dp.map(row => [...row]), rows, cols, current: [i,j], description: `dp[${i}][${j}] = dp[${i-1}][${j}] + dp[${i}][${j-1}] = ${dp[i-1][j]} + ${dp[i][j-1]} = ${dp[i][j]}`, pseudocodeLine: 3 });
      }
    }
    steps.push({ dp: dp.map(row => [...row]), rows, cols, result: dp[rows-1][cols-1], description: `Total paths = ${dp[rows-1][cols-1]}` });
    return steps;
  };

  useEffect(() => {
    setSteps(generateSteps(m, n));
    setCurrentStep(0);
    setIsPlaying(false);
  }, [m, n]);
  ...
  const rowLabels = Array.from({length: m}, (_,i) => `i=${i}`);
  const colLabels = Array.from({length: n}, (_,i) => `j=${i}`);

  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Unique Paths</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(m*n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(m*n)</span></div></div>
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        <input type="number" value={m} onChange={(e) => setM(Number(e.target.value))} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-24" min="1" />
        <input type="number" value={n} onChange={(e) => setN(Number(e.target.value))} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-24" min="1" />
      </div>
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          <DPTableVisualizer table={step.dp} rows={step.rows} cols={step.cols} rowLabels={rowLabels} colLabels={colLabels} highlight={{ current: step.current, computed: [] }} />

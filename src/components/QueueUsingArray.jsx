...
const QueueUsingArray = () => {
  const pseudocodeLines = [
    "class Queue:",
    "  arr = []",
    "  enqueue(x): arr.append(x)",
    "  dequeue(): return arr.pop(0) if arr else None",
    "  front(): return arr[0] if arr else None",
  ...
  const generateSteps = (ops) => {
    const steps = [];
    let queue = [];
    steps.push({ queue: [...queue], description: "Start with empty queue", pseudocodeLine: 0 });
    for (let i = 0; i < ops.length; i++) {
      const op = ops[i];
      if (op.startsWith('enqueue')) {
        const val = parseInt(op.split(' ')[1]);
        queue.push(val);
        steps.push({ queue: [...queue], description: `Enqueue ${val} → queue becomes [${queue.join(', ')}]`, pseudocodeLine: 2 });
      } else if (op === 'dequeue') {
        const dequeued = queue.length > 0 ? queue.shift() : null;
        steps.push({ queue: [...queue], description: `Dequeue → ${dequeued !== null ? dequeued : 'queue empty'}`, pseudocodeLine: 3 });
      } else if (op === 'front') {
        const frontVal = queue.length > 0 ? queue[0] : null;
        steps.push({ queue: [...queue], front: frontVal, description: `Front = ${frontVal !== null ? frontVal : 'queue empty'}`, pseudocodeLine: 4 });
      } else if (op === 'isEmpty') {
        const empty = queue.length === 0;
        steps.push({ queue: [...queue], empty, description: `isEmpty = ${empty}`, pseudocodeLine: 5 });
      }
    }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Queue using Array</h2>
        <div className="flex justify-center gap-4 mb-4">
          <span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Enqueue O(1), Dequeue O(n)</span>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">Queue (front ← → back)</div>
            <div className="flex gap-1">
              ...
              {step.queue?.length === 0 && <div className="text-gray-400">(empty)</div>}
            </div>
          </div>

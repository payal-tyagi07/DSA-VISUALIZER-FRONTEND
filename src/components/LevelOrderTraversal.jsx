...
const LevelOrderTraversal = () => {
  ...
  const generateSteps = (arr) => {
    ...
    const steps = [];
    if (!root) return [{ description: "Empty tree" }];
    const queue = [root];
    const result = [];
    let stepId = 0;
    steps.push({ root, queue: [root.val], result: [], description: "Start with root in queue", pseudocodeLine: 2 });
    while (queue.length) {
      ...
      for (let i = 0; i < levelSize; i++) {
        ...
        steps.push({ root, queue: queue.map(n=>n.val), result: [...result], highlight: new Set([node]), description: `Process node ${node.val}`, pseudocodeLine: 5 });
        if (node.left) {
          queue.push(node.left);
          steps.push({ root, queue: queue.map(n=>n.val), result: [...result], description: `Add left child ${node.left.val} to queue`, pseudocodeLine: 6 });
        }
        if (node.right) {
          queue.push(node.right);
          steps.push({ root, queue: queue.map(n=>n.val), result: [...result], description: `Add right child ${node.right.val} to queue`, pseudocodeLine: 7 });
        }
      }
      result.push(levelVals);
      steps.push({ root, queue: queue.map(n=>n.val), result: [...result], description: `Completed level: [${levelVals.join(',')}]`, pseudocodeLine: 4 });
    }
    steps.push({ root, result, description: `Level order traversal: ${result.map(l=>`[${l.join(',')}]`).join(' → ')}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Binary Tree Level Order Traversal</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]">
            ...
            <p className="text-sm text-gray-400">Queue: [{step.queue?.join(', ')}]</p>
            <p className="text-[#4ec9b0] mt-2">Level order result: {step.result?.map(l => `[${l.join(',')}]`).join(' → ')}</p>
          </div>

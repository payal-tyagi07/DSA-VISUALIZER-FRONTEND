...
const LongestSubstringKDistinct = () => {
  const pseudocodeLines = [
    "procedure longestSubstringKDistinct(s, k):",
    "  left = 0, maxLen = 0",
    "  freq = new Map()",
    "  for right = 0 to n-1:",
    ...
    "    while freq.size > k:",
    "      freq[s[left]]--",
    "      if freq[s[left]] == 0: freq.delete(s[left])",
    "      left++",
    "    maxLen = max(maxLen, right-left+1)",
    "  return maxLen"
  ];

  const [s, setS] = useState('eceba');
  const [k, setK] = useState(2);
  const [steps, setSteps] = useState([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1000);
  const [input, setInput] = useState('eceba');
  ...
  const generateSteps = (str, kVal) => {
    const steps = [];
    let left = 0;
    let maxLen = 0;
    const freq = new Map();
    steps.push({ str, k: kVal, left, right: -1, maxLen, freq: new Map(), description: "Start", pseudocodeLine: 1 });
    for (let right = 0; right < str.length; right++) {
      const ch = str[right];
      freq.set(ch, (freq.get(ch) || 0) + 1);
      steps.push({ str, k: kVal, left, right, freq: new Map(freq), description: `Add ${ch}, freq = ${JSON.stringify(Array.from(freq.entries()))}`, pseudocodeLine: 3 });
      while (freq.size > kVal) {
        const leftChar = str[left];
        const newCount = freq.get(leftChar) - 1;
        if (newCount === 0) freq.delete(leftChar);
        else freq.set(leftChar, newCount);
        steps.push({ str, k: kVal, left, right, freq: new Map(freq), description: `Remove ${leftChar} (freq becomes ${freq.size})`, pseudocodeLine: 4 });
        left++;
        steps.push({ str, k: kVal, left, right, freq: new Map(freq), description: `Move left to ${left}`, pseudocodeLine: 5 });
      }
      const len = right - left + 1;
      if (len > maxLen) {
        maxLen = len;
        steps.push({ str, k: kVal, left, right, maxLen, description: `New max length = ${maxLen} (substring ${str.slice(left, right+1)})`, pseudocodeLine: 6 });
      } else {
        steps.push({ str, k: kVal, left, right, maxLen, description: `Current window = ${str.slice(left, right+1)}, length = ${len}`, pseudocodeLine: 6 });
      }
    }
    steps.push({ result: maxLen, description: `Longest substring length = ${maxLen}`, pseudocodeLine: 7 });
    return steps;
  };

  useEffect(() => {
    setSteps(generateSteps(s, k));
    setCurrentStep(0);
    setIsPlaying(false);
  }, [s, k]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Longest Substring with K Distinct</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">String</div>
          ...
          <div>
            ...
            <div className="bg-[#1e1e1e] p-2 rounded">
              {step.freq && Array.from(step.freq.entries()).map(([ch, count]) => (
                <span key={ch} className="mr-3">{ch}:{count}</span>
              ))}
          ...
          {/* Progress, speed, buttons (same as previous) */}
          <div>
...
export default LongestSubstringKDistinct;
...
const LargestRectangleInHistogram = () => {
  const pseudocodeLines = [
    "procedure largestRectangleArea(heights):",
    "  stack = []",
    "  maxArea = 0",
    "  for i = 0 to n:",
    "    while stack not empty and (i == n or heights[stack.top()] > heights[i]):",
  ...
  const generateSteps = (arr) => {
    ...
    let stack = [];
    let maxArea = 0;
    steps.push({ heights: arr, stack: [...stack], maxArea, description: "Start", pseudocodeLine: 1 });
    for (let i = 0; i <= n; i++) {
      const currentHeight = i === n ? 0 : arr[i];
      steps.push({ heights: arr, stack: [...stack], maxArea, highlight: i, description: `Process index ${i} (height ${currentHeight})`, pseudocodeLine: 3 });
      while (stack.length > 0 && (i === n || arr[stack[stack.length-1]] > currentHeight)) {
        const height = arr[stack.pop()];
        const width = stack.length === 0 ? i : i - stack[stack.length-1] - 1;
        const area = height * width;
        maxArea = Math.max(maxArea, area);
        steps.push({ heights: arr, stack: [...stack], maxArea, area, description: `Pop index, height=${height}, width=${width}, area=${area}, new maxArea=${maxArea}`, pseudocodeLine: 4 });
      }
      if (i < n) {
        stack.push(i);
        steps.push({ heights: arr, stack: [...stack], maxArea, description: `Push index ${i} onto stack`, pseudocodeLine: 5 });
      }
    }
    steps.push({ result: maxArea, description: `Largest rectangle area = ${maxArea}`, pseudocodeLine: 6 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Largest Rectangle in Histogram</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          {/* Histogram bars */}
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">Histogram</div>
            <div className="flex items-end gap-1">
              {step.heights?.map((h, idx) => (
                <div key={idx} className="flex flex-col items-center">
                  <div className={`w-12 transition-all duration-300 ${idx === step.highlight ? 'bg-yellow-500' : 'bg-blue-500'}`} style={{ height: `${h * 20}px`, minHeight: '20px' }}>
                    <span className="text-white text-xs">{h}</span>
                  </div>
                  <div className="text-xs text-gray-400">idx {idx}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]">
            ...
            {step.area !== undefined && <p className="text-[#4ec9b0] mt-2">Area computed: {step.area}</p>}
            {step.result !== undefined && <p className="text-[#4ec9b0] mt-2">Maximum area = {step.result}</p>}
          </div>

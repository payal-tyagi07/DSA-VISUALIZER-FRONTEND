...
const BinaryTreeMaxPathSum = () => {
  ...
  const [input, setInput] = useState('-10,9,20,null,null,15,7');
  ...
  const generateSteps = (arr) => {
    ...
    const gain = (node) => {
      if (!node) return 0;
      steps.push({ root, highlight: new Set([node]), description: `At node ${node.val}, compute left and right gains` });
      const left = Math.max(0, gain(node.left));
      steps.push({ root, highlight: new Set([node]), description: `Left gain for ${node.val} = ${left}` });
      const right = Math.max(0, gain(node.right));
      steps.push({ root, highlight: new Set([node]), description: `Right gain for ${node.val} = ${right}` });
      const pathSum = left + right + node.val;
      if (pathSum > maxSum) {
        maxSum = pathSum;
        steps.push({ root, highlight: new Set([node]), description: `Path through ${node.val} = ${pathSum} → new max = ${maxSum}` });
      }
      const nodeGain = node.val + Math.max(left, right);
      steps.push({ root, highlight: new Set([node]), description: `Node gain = ${nodeGain}` });
      return nodeGain;
    };
    gain(root);
    steps.push({ root, maxSum, description: `Maximum path sum = ${maxSum}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Binary Tree Maximum Path Sum</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(n)</span></div></div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]"><p className="text-gray-300"><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {step.description}</p>{step.maxSum !== undefined && <p className="text-[#4ec9b0] mt-2">Max path sum = {step.maxSum}</p>}</div>

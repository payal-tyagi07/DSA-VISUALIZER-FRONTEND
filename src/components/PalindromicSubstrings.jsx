...
const PalindromicSubstrings = () => {
  ...
  const generateSteps = (str) => {
    ...
    let count = 0;
    steps.push({ dp: dp.map(row=>[...row]), n, description: "Initialize DP table", pseudocodeLine: 1 });
    for (let i = 0; i < n; i++) {
      dp[i][i] = true;
      count++;
      steps.push({ dp: dp.map(row=>[...row]), n, current: [i,i], count, description: `Single character at ${i} → palindrome, count = ${count}`, pseudocodeLine: 2 });
    }
    for (let i = 0; i < n-1; i++) {
      if (str[i] === str[i+1]) {
        dp[i][i+1] = true;
        count++;
        steps.push({ dp: dp.map(row=>[...row]), n, current: [i,i+1], count, description: `Two equal chars at ${i},${i+1} → palindrome, count = ${count}`, pseudocodeLine: 3 });
      } else {
        steps.push({ dp: dp.map(row=>[...row]), n, current: [i,i+1], count, description: `Chars ${str[i]} and ${str[i+1]} differ → no palindrome`, pseudocodeLine: 3 });
      }
    }
    for (let len = 3; len <= n; len++) {
      for (let i = 0; i <= n - len; i++) {
        ...
        if (str[i] === str[j] && dp[i+1][j-1]) {
          dp[i][j] = true;
          count++;
          steps.push({ dp: dp.map(row=>[...row]), n, current: [i,j], count, description: `Palindrome from ${i} to ${j} (${str.substring(i,j+1)}), count = ${count}`, pseudocodeLine: 4 });
        } else {
          steps.push({ dp: dp.map(row=>[...row]), n, current: [i,j], count, description: `Not palindrome from ${i} to ${j}`, pseudocodeLine: 4 });
        }
      }
    }
    steps.push({ dp: dp.map(row=>[...row]), n, result: count, description: `Total palindromic substrings = ${count}` });
    return steps;
  };

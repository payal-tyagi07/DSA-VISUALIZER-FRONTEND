...
const MaxHeap = () => {
  ...
  const [heap, setHeap] = useState([50, 30, 40, 10, 20, 35]);
  ...
  useEffect(() => {
    setSteps([{ heap: [...heap], description: "Initial max heap" }]);
    setCurrentStep(0);
    setIsPlaying(false);
  }, []);
  ...
  const generateStepsForAction = (currentHeap, actionType, value) => {
    ...
    if (actionType === 'insert' && value !== undefined && !isNaN(value)) {
      ...
      if (heap.length === 0) {
        steps.push({ heap: [...heap], description: "Heap is empty, cannot extract max" });
      } else {
        const maxVal = heap[0];
        steps.push({ heap: [...heap], description: `Extract max: ${maxVal}` });
        ...
        while (true) {
          ...
          if (largest !== index) {
            ...
            steps.push({ heap: [...heap], highlight: [index, largest], description: `Swap with larger child (${heap[index]})` });
            index = largest;
          } else break;
        }
        steps.push({ heap: [...heap], description: `Extract complete. New heap: [${heap.join(',')}], extracted: ${maxVal}` });
      }
    }
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Max Heap</h2>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        ...
        {action === 'extract' && (
          <button onClick={performAction} className="bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-1 rounded">Extract Max</button>
        )}
      </div>

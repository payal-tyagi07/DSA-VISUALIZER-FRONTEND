...
const Subsets = () => {
  ...
  const generateSteps = (arr) => {
    ...
    const backtrack = (start, path, depth = 0) => {
      steps.push({ type: 'call', label: `bt(${start})`, description: `Add subset ${JSON.stringify(path)}`, pseudocodeLine: 3, depth });
      for (let i = start; i < arr.length; i++) {
        steps.push({ type: 'choose', label: `+${arr[i]}`, description: `Include ${arr[i]}, path → ${JSON.stringify([...path, arr[i]])}`, pseudocodeLine: 5, depth: depth+1 });
        backtrack(i+1, [...path, arr[i]], depth+1);
        steps.push({ type: 'backtrack', label: `-${arr[i]}`, description: `Remove ${arr[i]}, path ← ${JSON.stringify(path)}`, pseudocodeLine: 7, depth: depth+1 });
      }
      steps.push({ type: 'return', label: `ret`, description: `Return`, pseudocodeLine: 8, depth });
    };
    backtrack(0, [], 0);
    steps.push({ type: 'result', description: `All subsets generated`, pseudocodeLine: 9 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Subsets</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ O(2ⁿ)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 O(n)</span></div></div>

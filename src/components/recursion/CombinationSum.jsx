...
const CombinationSum = () => {
  const pseudocodeLines = [
    "procedure combinationSum(candidates, target):",
    "  result = []",
  ...
  const [candidates, setCandidates] = useState([2,3,5]);
  const [target, setTarget] = useState(8);
  const [inputStr, setInputStr] = useState('[2,3,5],8');
  ...
  const generateSteps = (cand, tgt) => {
    ...
    const backtrack = (remain, start, path, depth = 0) => {
      steps.push({ type: 'call', label: `bt(${remain},${start})`, description: `Remain=${remain}, path=${JSON.stringify(path)}`, pseudocodeLine: 3, depth });
      if (remain === 0) {
        steps.push({ type: 'add', label: `sol`, description: `Found: ${JSON.stringify(path)}`, pseudocodeLine: 4, depth });
        steps.push({ type: 'return', label: `ret`, description: `Return`, depth, pseudocodeLine: 5 });
        return;
      }
      for (let i = start; i < cand.length; i++) {
        if (cand[i] <= remain) {
          steps.push({ type: 'choose', label: `+${cand[i]}`, description: `Take ${cand[i]}, remain→${remain-cand[i]}, path→${JSON.stringify([...path, cand[i]])}`, pseudocodeLine: 7, depth: depth+1 });
          backtrack(remain - cand[i], i, [...path, cand[i]], depth+1);
          steps.push({ type: 'backtrack', label: `-${cand[i]}`, description: `Remove ${cand[i]}, path←${JSON.stringify(path)}`, pseudocodeLine: 9, depth: depth+1 });
        } else {
          steps.push({ type: 'skip', label: `x${cand[i]}`, description: `Skip ${cand[i]} > remain`, pseudocodeLine: 7, depth: depth+1 });
        }
      }
      steps.push({ type: 'return', label: `ret`, description: `Return`, depth, pseudocodeLine: 10 });
    };
    backtrack(tgt, 0, [], 0);
    steps.push({ type: 'result', description: `All combinations found`, pseudocodeLine: 11 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Combination Sum</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ O(2ⁿ)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 O(target)</span></div></div>

...
const CombinationSumII = () => {
  const pseudocodeLines = [
    "procedure combinationSum2(candidates, target):",
    "  sort(candidates)",
    "  result = []",
  ...
  const [candidates, setCandidates] = useState([10,1,2,7,6,1,5]);
  const [target, setTarget] = useState(8);
  const [inputStr, setInputStr] = useState('[10,1,2,7,6,1,5],8');
  ...
  const updateParams = () => {
    try {
      const [candStr, tgtStr] = inputStr.split('],');
      setCandidates(JSON.parse(candStr + ']'));
      setTarget(parseInt(tgtStr));
    } catch(e) {}
  };

  const generateSteps = (cand, tgt) => {
    const sorted = [...cand].sort((a,b)=>a-b);
    const steps = [];
    const backtrack = (remain, start, path, depth = 0) => {
      steps.push({ type: 'call', label: `bt(${remain},${start})`, description: `Remain=${remain}, path=${JSON.stringify(path)}`, pseudocodeLine: 4, depth });
      if (remain === 0) {
        steps.push({ type: 'add', label: `sol`, description: `Found: ${JSON.stringify(path)}`, pseudocodeLine: 5, depth });
        steps.push({ type: 'return', label: `ret`, description: `Return`, depth, pseudocodeLine: 6 });
        return;
      }
      for (let i = start; i < sorted.length; i++) {
        if (i > start && sorted[i] === sorted[i-1]) {
          steps.push({ type: 'skip', label: `dup${sorted[i]}`, description: `Skip duplicate ${sorted[i]}`, pseudocodeLine: 8, depth: depth+1 });
          continue;
        }
        if (sorted[i] <= remain) {
          steps.push({ type: 'choose', label: `+${sorted[i]}`, description: `Take ${sorted[i]}, remain→${remain-sorted[i]}, path→${JSON.stringify([...path, sorted[i]])}`, pseudocodeLine: 9, depth: depth+1 });
          backtrack(remain - sorted[i], i+1, [...path, sorted[i]], depth+1);
          steps.push({ type: 'backtrack', label: `-${sorted[i]}`, description: `Remove ${sorted[i]}, path←${JSON.stringify(path)}`, pseudocodeLine: 11, depth: depth+1 });
        } else {
          steps.push({ type: 'skip', label: `x${sorted[i]}`, description: `Skip ${sorted[i]} > remain`, pseudocodeLine: 9, depth: depth+1 });
        }
      }
      steps.push({ type: 'return', label: `ret`, description: `Return`, depth, pseudocodeLine: 12 });
    };
    backtrack(tgt, 0, [], 0);
    steps.push({ type: 'result', description: `All combinations found`, pseudocodeLine: 13 });
    return steps;
  };

  useEffect(() => {
    setSteps(generateSteps(candidates, target));
    setCurrentStep(0);
    setIsPlaying(false);
  }, [candidates, target]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Combination Sum II</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ O(2ⁿ)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 O(n)</span></div></div>
      <div className="flex justify-center gap-4 mb-6"><label className="text-[#9cdcfe]">candidates, target:</label><input type="text" value={inputStr} onChange={(e) => setInputStr(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-64" /><button onClick={updateParams} className="bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-1 rounded">Update</button></div>

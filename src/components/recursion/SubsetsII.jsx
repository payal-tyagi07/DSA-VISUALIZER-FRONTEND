...
const SubsetsII = () => {
  ...
  const [inputStr, setInputStr] = useState('[1,2,2]');
  ...
  const generateSteps = (arr) => {
    ...
    const backtrack = (start, path, depth = 0) => {
      steps.push({ type: 'call', label: `bt(${start})`, description: `Add subset ${JSON.stringify(path)}`, pseudocodeLine: 4, depth });
      for (let i = start; i < sorted.length; i++) {
        if (i > start && sorted[i] === sorted[i-1]) {
          steps.push({ type: 'skip', label: `dup${sorted[i]}`, description: `Skip duplicate ${sorted[i]}`, pseudocodeLine: 6, depth: depth+1 });
          continue;
        }
        steps.push({ type: 'choose', label: `+${sorted[i]}`, description: `Include ${sorted[i]}, path → ${JSON.stringify([...path, sorted[i]])}`, pseudocodeLine: 7, depth: depth+1 });
        backtrack(i+1, [...path, sorted[i]], depth+1);
        steps.push({ type: 'backtrack', label: `-${sorted[i]}`, description: `Remove ${sorted[i]}, path ← ${JSON.stringify(path)}`, pseudocodeLine: 9, depth: depth+1 });
      }
      steps.push({ type: 'return', label: `ret`, description: `Return`, pseudocodeLine: 10, depth });
    };
    backtrack(0, [], 0);
    steps.push({ type: 'result', description: `All subsets generated`, pseudocodeLine: 11 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Subsets II</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ O(2ⁿ)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 O(n)</span></div></div>

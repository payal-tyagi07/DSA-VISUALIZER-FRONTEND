...
const PostorderTraversal = () => {
  ...
  const generateSteps = (arr) => {
    ...
    const postorder = (node) => {
      if (!node) return;
      steps.push({ root, highlight: new Set([node]), description: `Go left from ${node.val}` });
      postorder(node.left);
      steps.push({ root, highlight: new Set([node]), description: `Go right from ${node.val}` });
    ...
    postorder(root);
    steps.push({ root, visited: new Set(visitedSet), result, description: `Postorder traversal: ${result.join(', ')}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Binary Tree Postorder Traversal</h2>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          {step.root && <TreeRenderer root={step.root} highlightNodes={step.highlight} visitedNodes={step.visited} />}

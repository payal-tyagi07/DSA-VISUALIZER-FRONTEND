...
const DailyTemperatures = () => {
  const pseudocodeLines = [
    "procedure dailyTemperatures(temps):",
    "  n = len(temps)",
    "  result = [0]*n",
    "  stack = []",
    "  for i = 0 to n-1:",
  ...
  const updateTemps = () => {
    ...
    if (newArr.length) setTemps(newArr);
  };
  ...
  const generateSteps = (arr) => {
    ...
    let stack = [];
    steps.push({ temps: arr, result: [...result], stack: [...stack], description: "Start", pseudocodeLine: 1 });
    for (let i = 0; i < n; i++) {
      steps.push({ temps: arr, result: [...result], stack: [...stack], highlight: i, description: `Processing index ${i} (temp ${arr[i]})`, pseudocodeLine: 3 });
      while (stack.length > 0 && arr[stack[stack.length-1]] < arr[i]) {
        const idx = stack.pop();
        const days = i - idx;
        result[idx] = days;
        steps.push({ temps: arr, result: [...result], stack: [...stack], highlight: i, description: `For index ${idx}, warmer after ${days} days`, pseudocodeLine: 4 });
      }
      stack.push(i);
      steps.push({ temps: arr, result: [...result], stack: [...stack], highlight: i, description: `Push index ${i} onto stack`, pseudocodeLine: 5 });
    }
    steps.push({ result, description: `Days to warmer: ${result.join(', ')}`, pseudocodeLine: 6 });
    return steps;
  };

  useEffect(() => {
    setSteps(generateSteps(temps));
    setCurrentStep(0);
    setIsPlaying(false);
  }, [temps]);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Daily Temperatures</h2>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        ...
        <button onClick={updateTemps} className="bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-1 rounded">Update</button>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          {/* Temperatures array */}
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">Temperatures</div>
            <div className="flex gap-1">
              {step.temps?.map((val, idx) => (
          ...
          {/* Result array */}
          <div>
            <div className="text-sm text-[#9cdcfe] mb-1">Days to warmer</div>
            <div className="flex gap-1">
              {step.result?.map((val, idx) => (
                <div key={idx} className="w-12 h-12 flex items-center justify-center text-lg font-mono rounded border bg-green-500 text-white border-[#3c3c3c]">
                  {val}
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex gap-1">
              {step.stack?.map((idx, i) => (
                <div key={i} className="w-12 h-12 flex items-center justify-center text-lg font-mono rounded border bg-blue-500 text-white border-[#3c3c3c]">
                  {idx}
                </div>
              ))}

...
const MaxSubarray = () => {
  ...
  const generateSteps = (arr) => {
    ...
    steps.push({ maxSoFar, maxEndingHere, current: 0, description: "Initialize with first element", pseudocodeLine: 1 });
    for (let i = 1; i < arr.length; i++) {
      const prevMax = maxEndingHere;
      maxEndingHere = Math.max(arr[i], maxEndingHere + arr[i]);
      maxSoFar = Math.max(maxSoFar, maxEndingHere);
      steps.push({ maxSoFar, maxEndingHere, current: i, arr, description: `i=${i}: maxEndingHere = max(${arr[i]}, ${prevMax}+${arr[i]}) = ${maxEndingHere}, maxSoFar = ${maxSoFar}`, pseudocodeLine: 2 });
    }
    steps.push({ result: maxSoFar, description: `Maximum subarray sum = ${maxSoFar}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Maximum Subarray (Kadane's Algorithm)</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(1)</span></div></div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#1e1e1e] p-3 rounded"><p className="text-[#9cdcfe]">maxEndingHere = {step.maxEndingHere}, maxSoFar = {step.maxSoFar}</p></div>

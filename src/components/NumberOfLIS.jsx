...
const NumberOfLIS = () => {
  const pseudocodeLines = [
    "procedure findNumberOfLIS(nums):",
    "  n = len(nums)",
    "  length = array of size n, fill with 1",
    "  count = array of size n, fill with 1",
    "  for i = 1 to n-1:",
    "    for j = 0 to i-1:",
    "      if nums[j] < nums[i]:",
    ...
    "          count[i] += count[j]",
    "  maxLen = max(length)",
    "  result = sum(count[i] for i where length[i]==maxLen)",
    "  return result"
  ];
  const [nums, setNums] = useState([1,3,5,4,7]);
  const [steps, setSteps] = useState([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1000);
  const [input, setInput] = useState('1,3,5,4,7');
  ...
  const generateSteps = (arr) => {
    ...
    steps.push({ length: [...length], count: [...count], description: "Initialize length and count arrays", pseudocodeLine: 1 });
    for (let i = 1; i < n; i++) {
      for (let j = 0; j < i; j++) {
        if (arr[j] < arr[i]) {
          if (length[j] + 1 > length[i]) {
            length[i] = length[j] + 1;
            count[i] = count[j];
            steps.push({ length: [...length], count: [...count], current: i, j, description: `New longer LIS at i=${i} from j=${j}: length=${length[i]}, count=${count[i]}`, pseudocodeLine: 5 });
          } else if (length[j] + 1 === length[i]) {
            count[i] += count[j];
            steps.push({ length: [...length], count: [...count], current: i, j, description: `Same length at i=${i} from j=${j}: count becomes ${count[i]}`, pseudocodeLine: 6 });
          }
        }
      }
    }
    const maxLen = Math.max(...length);
    const result = length.reduce((sum, len, idx) => len === maxLen ? sum + count[idx] : sum, 0);
    steps.push({ length: [...length], count: [...count], maxLen, result, description: `Number of LIS of length ${maxLen} = ${result}` });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Number of Longest Increasing Subsequence</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(n²)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(n)</span></div></div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          <div className="flex gap-2 flex-wrap justify-center mb-4">
            {nums.map((val, i) => <div key={i} className="w-12 h-12 flex items-center justify-center text-lg font-mono rounded border bg-[#1e1e1e] text-white border-[#3c3c3c]">{val}</div>)}
          </div>
          <div className="bg-[#1e1e1e] p-3 rounded mb-2"><p className="text-[#9cdcfe]">Length array</p><DPArrayVisualizer array={step.length} labels={Array.from({length: step.length?.length}, (_,i)=>i)} highlight={{ current: step.current }} /></div>
          <div className="bg-[#1e1e1e] p-3 rounded"><p className="text-[#9cdcfe]">Count array</p><DPArrayVisualizer array={step.count} labels={Array.from({length: step.count?.length}, (_,i)=>i)} highlight={{ current: step.current }} /></div>

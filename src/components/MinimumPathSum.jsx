...
const MinimumPathSum = () => {
  const pseudocodeLines = [
    "procedure minPathSum(grid):",
    "  m = rows, n = cols",
    "  dp = 2D array copy of grid",
    "  for i = 1 to m-1: dp[i][0] += dp[i-1][0]",
    "  for j = 1 to n-1: dp[0][j] += dp[0][j-1]",
    "  for i = 1 to m-1:",
    "    for j = 1 to n-1:",
    "      dp[i][j] += min(dp[i-1][j], dp[i][j-1])",
    "  return dp[m-1][n-1]"
  ];
  const [grid, setGrid] = useState([[1,3,1],[1,5,1],[4,2,1]]);
  const [steps, setSteps] = useState([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isPlaying, setIsPlaying] = useState(false);
  const [speed, setSpeed] = useState(1000);
  const [input, setInput] = useState('1,3,1;1,5,1;4,2,1');
  ...
  const generateSteps = (grid) => {
    const steps = [];
    const m = grid.length, n = grid[0].length;
    const dp = grid.map(row => [...row]);
    steps.push({ dp: dp.map(row => [...row]), m, n, description: "Initial grid (cost to reach each cell)", pseudocodeLine: 1 });
    for (let i = 1; i < m; i++) {
      dp[i][0] += dp[i-1][0];
      steps.push({ dp: dp.map(row => [...row]), m, n, current: [i,0], description: `First column: dp[${i}][0] = ${dp[i][0]}`, pseudocodeLine: 2 });
    }
    for (let j = 1; j < n; j++) {
      dp[0][j] += dp[0][j-1];
      steps.push({ dp: dp.map(row => [...row]), m, n, current: [0,j], description: `First row: dp[0][${j}] = ${dp[0][j]}`, pseudocodeLine: 3 });
    }
    for (let i = 1; i < m; i++) {
      for (let j = 1; j < n; j++) {
        const minPrev = Math.min(dp[i-1][j], dp[i][j-1]);
        dp[i][j] += minPrev;
        steps.push({ dp: dp.map(row => [...row]), m, n, current: [i,j], description: `dp[${i}][${j}] = ${dp[i][j]} (min(${dp[i-1][j]}, ${dp[i][j-1]}) + grid[${i}][${j}])`, pseudocodeLine: 5 });
      }
    }
    steps.push({ dp: dp.map(row => [...row]), m, n, result: dp[m-1][n-1], description: `Minimum path sum = ${dp[m-1][n-1]}` });
    return steps;
  };
  ...
  const rowLabels = Array.from({length: step.m}, (_,i)=>i);
  const colLabels = Array.from({length: step.n}, (_,i)=>i);

  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Minimum Path Sum</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(m*n)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(m*n)</span></div></div>
      <div className="flex flex-wrap justify-center gap-4 mb-6"><input type="text" value={input} onChange={(e) => setInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-80" /><button onClick={updateGrid} className="bg-[#0e639c] hover:bg-[#1177bb] text-white px-4 py-1 rounded">Update</button></div>
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          <DPTableVisualizer table={step.dp} rows={step.m} cols={step.n} rowLabels={rowLabels} colLabels={colLabels} highlight={{ current: step.current }} />

...
const ReverseLinkedList = () => {
  ...
  const generateSteps = (arr) => {
    ...
    let curr = 0;
    let currentArr = [...arr];
    steps.push({ array: currentArr, prev, curr, description: "Start: prev = null, curr = head", pseudocodeLine: 1 });
    while (curr < currentArr.length) {
      const nextTemp = curr + 1;
      steps.push({ array: currentArr, prev, curr, next: nextTemp, description: `Store next = index ${nextTemp} (value ${nextTemp < currentArr.length ? currentArr[nextTemp] : 'null'})`, pseudocodeLine: 3 });
      steps.push({ array: currentArr, prev, curr, next: nextTemp, description: `Reverse link: node ${curr} now points to ${prev !== null ? prev : 'null'}`, pseudocodeLine: 4 });
      prev = curr;
      curr = nextTemp;
      if (curr < currentArr.length) {
        steps.push({ array: currentArr, prev, curr, description: `Move prev to ${prev}, curr to ${curr}`, pseudocodeLine: 5 });
      }
    }
    const reversed = [...currentArr].reverse();
    steps.push({ array: reversed, prev: null, curr: null, description: `Reversed list: [${reversed.join(',')}]`, pseudocodeLine: 6 });
    return steps;
  };
  ...
  const getNodeStyle = (idx) => {
    if (step.curr === idx) return 'border-2 border-yellow-500 bg-yellow-900/30';
    if (step.prev === idx) return 'border-2 border-green-500 bg-green-900/30';
              ...
              {step.array?.map((val, idx) => (
                <div key={idx} className="relative flex items-center">
                  ...
                  <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs whitespace-nowrap">
                    {step.curr === idx && <span className="text-yellow-500">curr</span>}
                    {step.prev === idx && <span className="text-green-500">prev</span>}
                  </div>
                </div>
              ))}

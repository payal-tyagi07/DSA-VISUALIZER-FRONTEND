...
const DetectCycle = () => {
  ...
  const generateSteps = (arr, cycleIdx) => {
    const steps = [];
    let slow = 0, fast = 0;
    steps.push({ array: arr, slow, fast, cycleIdx, description: "Start: slow = head, fast = head", pseudocodeLine: 1 });
    let meeting = -1;
    while (fast < arr.length && fast + 1 < arr.length) {
      slow = (slow + 1) % arr.length;
      fast = (fast + 2) % arr.length;
      steps.push({ array: arr, slow, fast, cycleIdx, description: `Move slow to ${slow}, fast to ${fast}`, pseudocodeLine: 3 });
      if (slow === fast) {
        meeting = slow;
        steps.push({ array: arr, slow, fast, cycleIdx, result: true, description: `Cycle detected at index ${meeting}`, pseudocodeLine: 5 });
        break;
      }
    }
    if (meeting === -1) {
      steps.push({ array: arr, cycleIdx, result: false, description: "No cycle detected", pseudocodeLine: 6 });
    }
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6">
        <h2 className="text-3xl font-bold text-[#569cd6] mb-2">Detect Cycle (Floyd's Algorithm)</h2>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        ...
        <input type="number" value={cycleInput} onChange={(e) => setCycleInput(e.target.value)} className="bg-[#1e1e1e] border border-[#3c3c3c] rounded px-3 py-1 w-20" placeholder="Cycle start index (-1 for none)" />

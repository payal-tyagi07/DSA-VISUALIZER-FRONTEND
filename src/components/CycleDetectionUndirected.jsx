...
const CycleDetectionUndirected = () => {
  const pseudocodeLines = [
    "procedure hasCycleUndirected(graph):",
    "  visited = set()",
    "  for each node in graph:",
    "    if node not in visited:",
    "      if dfs(node, parent=-1): return true",
    "  return false",
  ...
  const graph = {
    ...
    edges: [
      ...
      { from: 'D', to: 'E' } // no cycle, add an extra edge to test: { from: 'E', to: 'B' } would create cycle
    ]
  };
  ...
  const generateSteps = () => {
    ...
    const dfs = (node, parent, depth = 0) => {
      visited.add(node);
      steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), currentNode: node, description: `Visit ${node}`, pseudocodeLine: 6 });
      const neighbors = graph.edges.filter(e => e.from === node).map(e => e.to).concat(graph.edges.filter(e => e.to === node).map(e => e.from));
      for (const neighbor of neighbors) {
        if (!visited.has(neighbor)) {
          steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), currentNode: node, highlightNodes: new Set([neighbor]), description: `Check neighbor ${neighbor} (not visited)`, pseudocodeLine: 7 });
          if (dfs(neighbor, node, depth+1)) return true;
        } else if (neighbor !== parent) {
          steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), currentNode: node, highlightNodes: new Set([neighbor]), description: `Found back edge to ${neighbor} (already visited, not parent) → cycle detected`, pseudocodeLine: 8 });
          return true;
        }
      }
      return false;
    ...
    steps.push({ nodes: graph.nodes, edges: graph.edges, visited, cycleFound, description: `Graph ${cycleFound ? 'contains' : 'does not contain'} a cycle`, pseudocodeLine: 4 });
    return steps;
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      <div className="text-center mb-6"><h2 className="text-3xl font-bold text-[#569cd6] mb-2">Cycle Detection (Undirected Graph)</h2><div className="flex justify-center gap-4 mb-4"><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">⏱️ Time: O(V+E)</span><span className="bg-[#2d2d2d] px-3 py-1 rounded-full text-sm">💾 Space: O(V)</span></div></div>
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]"><p className="text-gray-300"><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {step.description}</p>{step.cycleFound !== undefined && <p className="text-[#4ec9b0] mt-2">Result: {step.cycleFound ? 'Cycle detected' : 'No cycle'}</p>}</div>
...
export default CycleDetectionUndirected;
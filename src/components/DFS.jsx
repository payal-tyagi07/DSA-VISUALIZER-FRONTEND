...
const DFS = () => {
  ...
  const generateSteps = () => {
    ...
    steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), stack: [...stack], description: `Start DFS from ${startNode}`, pseudocodeLine: 1 });
    while (stack.length) {
      const u = stack.pop();
      steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), stack: [...stack], currentNode: u, description: `Pop ${u} from stack`, pseudocodeLine: 3 });
      if (!visited.has(u)) {
        visited.add(u);
        steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), stack: [...stack], highlightNodes: new Set([u]), description: `Visit ${u}`, pseudocodeLine: 4 });
        const neighbors = graph.edges.filter(e => e.from === u).map(e => e.to).concat(graph.edges.filter(e => e.to === u).map(e => e.from));
        for (const v of neighbors) {
          if (!visited.has(v)) {
            stack.push(v);
            steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), stack: [...stack], description: `Push neighbor ${v} onto stack`, pseudocodeLine: 5 });
          }
        }
      }
    }
    steps.push({ nodes: graph.nodes, edges: graph.edges, visited: new Set(visited), description: `DFS traversal order: ${Array.from(visited).join(' → ')}`, pseudocodeLine: 7 });
    return steps;
  };
    ...
    <div className="max-w-6xl mx-auto p-6 bg-[#0a0a0a] text-gray-200 rounded-2xl shadow-2xl font-mono border border-[#222222]">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="lg:w-2/3 space-y-6">
          ...
          <div className="bg-[#0d0d0d] p-4 rounded-lg border-l-4 border-[#569cd6]"><p className="text-gray-300"><span className="font-bold text-[#9cdcfe]">Step {currentStep+1}:</span> {step.description}</p><p className="text-sm text-gray-400">Stack: [{step.stack?.join(', ')}]</p></div>
